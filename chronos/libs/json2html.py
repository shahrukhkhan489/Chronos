...
class Json2Html:
    ...
    def __init__(self):
        self.table_init_markup = '<table border="1">'
        self.clubbing = True
        self.encode = False
        self.escape = True
        self.collapsible_sub_tables = True
        self.toggle_id = 0

    def convert(self, json="", table_attributes='border="1"', clubbing=True, encode=False, escape=True, collapsible_sub_tables=True):
        """
            Convert JSON to HTML Table format
        """
        self.collapsible_sub_tables = collapsible_sub_tables
        self.escape = escape
    ...
    def convert_list(self, list_input):
        ...
        if column_headers is not None:
            # toggle_id = 'blavla'
            # converted_output += '<a id="{}" class="link-more" style="font-weight: bold; display: block;" onclick="this.style.display=\'none\'; document.getElementById(\'states\').style.display=\'block\'; return false;">SHOW</a>'.format(toggle_id)
    ...
    def convert_object(self, json_input):
        ...
        converted_output = ""
        if self.collapsible_sub_tables:
            self.toggle_id += 1
            converted_output += '<button id="toggle_show_{}" class="collapsible" style="font-weight: bold; display: block;" onclick="this.style.display=\'none\'; document.getElementById(\'table_{}\').style.display=\'block\'; document.getElementById(\'toggle_hide_{}\').style.display=\'block\'; return false;">Show</button>'.format(self.toggle_id, self.toggle_id, self.toggle_id)
            converted_output += '<button id="toggle_hide_{}" class="collapsible" style="font-weight: bold; display: none;" onclick="this.style.display=\'none\'; document.getElementById(\'table_{}\').style.display=\'none\'; document.getElementById(\'toggle_show_{}\').style.display=\'block\'; return false;">Hide</button>'.format(self.toggle_id, self.toggle_id, self.toggle_id)
            converted_output += self.table_init_markup.replace('<table', '<table id="table_{}" style="display: none;"'.format(self.toggle_id)) + "<tr>"
        else:
